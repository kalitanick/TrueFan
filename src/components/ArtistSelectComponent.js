...
function ArtistSelectComponent(props) {
    const [filter_live, setFilterLive] = useState(true);
    const [filter_short, setFilterShort] = useState(true);
    const [filter_commentary, setFilterCommentary] = useState(true);
    ...
    render() {
        return (
            <div className="ArtistSelect" style={{ backgroundColor: this.state.creating_playlist ? 'rgba(0, 0, 0, .4)' : null }}>
                ...
                {/*Error in Search*/this.state.has_searched && this.state.artist_search !== "" && this.state.artist_name === "" && (
                    <ErrorMessage errorMessage="Spotify couldn't find an artist with that name" />
                )}
                {/*No Artist Entered*/this.state.has_searched && this.state.artist_search === "" && (
                    <ErrorMessage errorMessage="Please Enter an Artist Name" />
                )}
                {/*Successful Search*/this.state.least_popular_tracks && (
                    <div>
                        <div>
